...
// SQLite 版 schema，必须与 schema.pg.ts 保持一致的表名/列名/默认值：
// - pg bigint(mode: number) / integer → sqlite integer（SQLite 整数本身是 64 位）
// - pg serial 主键 → sqlite integer primary key autoincrement
// 改表结构时两个文件都要改，并分别生成迁移（db:generate 与 db:generate:sqlite）。
...
export const consoleRequests = sqliteTable('console_requests', {
  ...
  firstChunkAt: integer('first_chunk_at'),
  firstTokenAt: integer('first_token_at'),
  ...
  quotaChargedMicrousd: integer('quota_charged_microusd').notNull().default(0),
...
export const consoleApiKeys = sqliteTable('console_api_keys', {
  ...
  costQuotaMicrousd: integer('cost_quota_microusd'),
  costUsedMicrousd: integer('cost_used_microusd').notNull().default(0),
}, (table) => ({
...
export const consoleProviders = sqliteTable('console_providers', {
  ...
  modelsSyncedAt: integer('models_synced_at'),
  createdAt: integer('created_at').notNull(),
