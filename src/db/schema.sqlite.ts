...
// SQLite mirror of ./schema.pg.ts. Column names, defaults and indexes are kept
// identical so the shared Drizzle query-builder code in the stores works against
// either dialect. Type mapping vs PostgreSQL:
//   pg `bigint({mode:'number'})` → sqlite `integer({mode:'number'})` (64-bit int)
//   pg `serial`                  → sqlite `integer(primaryKey autoIncrement)`
//   pg `integer` / `text`        → sqlite `integer` / `text`

export const consoleRequests = sqliteTable('console_requests', {
  ...
  quotaChargedMicrousd: integer('quota_charged_microusd', { mode: 'number' }).notNull().default(0),
...
export const consoleApiKeys = sqliteTable('console_api_keys', {
  ...
  costQuotaMicrousd: integer('cost_quota_microusd', { mode: 'number' }),
  costUsedMicrousd: integer('cost_used_microusd', { mode: 'number' }).notNull().default(0),
}, (table) => ({
...
export const consoleProviders = sqliteTable('console_providers', {
...
export const modelMetadataOverrides = sqliteTable('model_metadata_overrides', {
  id: integer('id').primaryKey({ autoIncrement: true }),
